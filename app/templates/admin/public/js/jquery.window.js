...
$.Window = (function()  {
	...
	var ICON_WH = 20;               // window icon button width/height, in pixels. check "window_icon_button" style in css
	...
	// get the window instance
	function getWindowByTitle(windowTitle) {
		for( var i=0, len=windowStorage.length; i<len; i++ ) {
			var wnd = windowStorage[i];
			//alert(wnd.getTitle());
			if( wnd.getTitle() == windowTitle ) {
				return wnd;
			}
		}
	}
	...
	function constructor(caller, options) {
		...
		var header = null;                // the header panel of window. it require_onces title text and buttons
		...
		function initialize(instance) {
			...
			// add close button
			if( options.closable ) {
				headerFuncPanel.append('<a href="javascript:void(0)" class="closeImg window_icon_button no-draggable"><i title="close window" class="fa fa-times"></i></a>');
			...
			// add maximize button
			if( options.maximizable ) {
				headerFuncPanel.append('<a href="javascript:void(0)" class="maximizeImg window_icon_button no-draggable"><i title="maximize window" class="fa fa-expand"></i></a>');
				headerFuncPanel.append('<a href="javascript:void(0)" class="cascadeImg window_icon_button no-draggable" style="display:none;"><i title="cascade window" class="fa fa-compress"></i></a>');
			...
			if( options.minimizable ) {
				headerFuncPanel.append('<a href="javascript:void(0)" class="minimizeImg window_icon_button no-draggable"><i title="minimize window" class="fa fa-minus"></i></a>');
			...
			if( options.bookmarkable/* && options.url != null && $.trim(options.url) != ""*/ ) {
				headerFuncPanel.append('<a href="javascript:void(0)" class="bookmarkImg window_icon_button no-draggable"><i title="Refresh" class="fa fa-star"></i></a>');
			...
			if( options.url != null && $.trim(options.url) != "" ) {
				...
				container.append("<div class='window_frame ui-widget-content no-draggable no-resizable "+options.frameClass+"' style='width:100%; height:"+(frameHeight-5)+"px;'></div>");
				...
				$.ajax({
                    ...
                    error: function (xhr) {
                        ...
                        if (json.error) {
                            jAlert(json.error.message + ' at line: ' + json.error.line + ' file: ' + json.error.file);
                            close();
                        }
                    }
				});
		...
		function refreshWindow() {
			console.log(frame);
			frame.append('<div class="overlay"><div class="frame_loading"></div></div>');
			var loading = frame.children(".frame_loading");
			...
			$.ajax({
				url: options.url,
			  	...
			  	success: function(html) {
					frame.empty();
	          		if (html.failed) {
	            		frame.append('<div class="flash flash-error">' + html.error + '</div>');
	            		resize(options.width, 63);
	          		} else {
	  					frame.append(html);
	          		}
	          		options.onLoaded(_this);
		...
		function getFrameHeight(windowHeight) {
			...
			return windowHeight - 20 - footerHeight; // minus header & footer & iframe's padding height
		}
		...
		function select() {
			...
			//if( maximized == false ) {
				container.css('z-index', index_highest+1);
		...
		function maximize(bImmediately, bNoSaveDisplay) {
			...
			if( caller != null ) {
				...
				targetCssStyle = {
					left: cpos.left,
					top: cpos.top,
					width: caller.width() - 16,
					height: caller.height() - 16,
					opacity: 1
				};
			} else {
				targetCssStyle = {
		...
		function restore()
		{
			...
			var zIndex = index_highest+1;
			
			
			if( minimized )
			{ // from minimized status
				rpos = orgPos;
				rwh = orgWh;
				if( caller == null )
				{
		...
		return { // instance public methods
			...
			setWindowId: function(id) {                     // get window id
				windowId = id;
			},
	...
	return { // static public methods
		...
		getWindowByTitle: getWindowByTitle, // get the window instance by passed window title
		refreshAllWindows: refreshAllWindows,
...
$.window.getWindowByTitle = $.Window.getWindowByTitle;
$.window.getSelectedWindow = $.Window.getSelectedWindow;
$.window.refreshAllWindows = $.Window.refreshAllWindows;